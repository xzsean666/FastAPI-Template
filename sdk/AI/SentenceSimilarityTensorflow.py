...
from tensorflow_hub import load as load_model

from ..DB.db_sqlite import KVDB

cache_db = KVDB("sqlite_cache.db", "sentence_similarity")


class SentenceSimilarity:
    def __init__(self, model_path=None, cache_expiration=None):
        self.model = None
        self.model_url = "https://tfhub.dev/google/universal-sentence-encoder/4"
        self.model_path = model_path
        self.cache_expiration = cache_expiration  # 缓存过期时间（秒）
    ...
    def init(self):
        if self.model is None:
            ...
            try:
                # Try loading with GPU support first
