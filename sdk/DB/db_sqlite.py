...
class KVDB:
    def __init__(
        self, datasource_or_url: Optional[str] = None, table_name: str = "kv_store"
    ):
        ...
        with self.Session() as session:
            ...
            if record:
                ...
                new_record = self.CustomKVStore(
                    key=key, value=json.dumps(value, default=bigint_handler)
                )
    ...
    async def get(
        self, key: str, expire: Optional[int] = None, delete: bool = False
    ) -> Any:
        await self.ensure_initialized()
        ...
        for i in range(0, len(entries), batch_size):
            ...
            with self.Session() as session:
                for key, value in batch:
                    record = (
                        session.query(self.CustomKVStore).filter_by(key=key).first()
                    )
    ...
    async def delete_many(self, keys: List[str]) -> int:
        ...
        with self.Session() as session:
            count = (
                session.query(self.CustomKVStore)
                .filter(self.CustomKVStore.key.in_(keys))
    ...
    async def find_by_value(self, value: Any, exact: bool = True) -> List[str]:
        ...
        with self.Session() as session:
            if exact:
                ...
                records = (
                    session.query(self.CustomKVStore)
                    .filter(self.CustomKVStore.value == value_str)
                    .all()
                )
            else:
                search_value = (
                    ...
                    else json.dumps(value, default=bigint_handler)
                )
                records = (
                    session.query(self.CustomKVStore)
                    .filter(self.CustomKVStore.value.like(f"%{search_value}%"))
                    .all()
                )
